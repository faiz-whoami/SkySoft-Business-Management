...
namespace business_managment_system.Data
{
    ...
    {
        public IList<Supplier> Search(string name, bool? isActive)
        {
            const string sql = @"
                SELECT SupplierId, Name, Email, Phone, Address, IsActive, CreatedDate, ModifiedDate
                FROM dbo.Supplier
        ...
        {
            const string sql = @"
                INSERT INTO dbo.Supplier (Name, Email, Phone, Address, IsActive)
        ...
        public IList<PartyTransactionRow> GetTransactions(int supplierId)
        {
            const string sql = @"
        ...
        private static void AddSaveParameters(SqlCommand command, Supplier supplier)
        {
            command.Parameters.AddWithValue("@Name", supplier.Name);
            command.Parameters.AddWithValue("@Email", (object)NullIfEmpty(supplier.Email) ?? DBNull.Value);
            command.Parameters.AddWithValue("@Phone", (object)NullIfEmpty(supplier.Phone) ?? DBNull.Value);
            command.Parameters.AddWithValue("@Address", (object)NullIfEmpty(supplier.Address) ?? DBNull.Value);
        }

        private static Supplier Map(IDataRecord reader)
        {
            return new Supplier
            {
